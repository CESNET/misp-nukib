...
* `OIDC_DISABLE_REQUEST_OBJECT` (optional, boolean, default `false`) - set to `true` to disable the Request Object approach in authorization requests, allowing users to fallback to plain parameters when needed for compatibility with certain OIDC providers.
